...
public class VS3Messages {
	 ...
	 * @param input the stream to be checked
	 * @return the sender's class
	 * @throws IllegalArgumentException
	 */
	public static char getStationClass(byte[] input) throws IllegalArgumentException{
		if(input.length!=MESSAGE_SIZE){
			throw new IllegalArgumentException("A valid message needs to have 34 bytes");
		}
		if(input[ZERO_IDX]=='A'){
			return 'A';
			...
			throw new IllegalArgumentException("Station type needs to be 'A' or 'B'");
		}
	}
	...
	public static long getSendTime(byte[] input){
		...
		byte[] time = Arrays.copyOfRange(input, SEND_TIME_START,MESSAGE_SIZE);
		return Long.parseLong(new String(time));
	}
	
	/**
	 * Creates a new message according to protocol 
	 * @param type	the sending station's type
	 * @param message	the payload as a String
	 * @param nextSlot	the next slot intended to be used by sender
	 * @param sendTime	the time the message was sent
	 * @return	the message created
	 */
	public static byte[] getMessage(char type, String payload, byte nextSlot, long sendTime){
		byte[] sendTimeBytes;
		...
		//write payload
		System.arraycopy(payload.getBytes(), ZERO_IDX, msg, PAYLOAD_START, payload.length());
				
		//write nextSlot
		msg[NEXT_SLOT_IDX]=(byte)nextSlot;
		
		//write sendtime
		System.arraycopy(sendTimeBytes, ZERO_IDX, msg, SEND_TIME_START, sendTimeBytes.length);
...
//		return reverseMsg;
	 ...
	 * @return	the message created
	 */
	public static byte[] getMessage(char type, byte[] payload, byte nextSlot, long sendTime){
		byte[] sendTimeBytes;
		byte[] msg = new byte[MESSAGE_SIZE];
