...
#define MODE "client" // "server" // "manager" // "client"
...
//    int512_t evaluate(coordinatorState& state) {
//        return state;
//    }
...
//persistant memory for managers
    struct managerState
    {
        ...
        int512_t cycles_last = 0;
        int512_t last_factor = 0;
    };
        ...
        return
                {
                        ...
                        [=](numberToSplit nts){
                            if(nts.p != TERMINATING_VALUE){
                                cerr << " send if nts.p !=  TERMINATING_VALUE" << endl;
                                self->state.cycles_last = nts.cycles;
...
//                        },
                        ...
                        [=](request_atom, int512_t N)
                        {
                            ...
                            {
                                cerr << self->state.num.N << " is a prime" << endl;
                        ...
                        [=](deliver_atom, numberToSplit nts)
                        {
                            ...
                            // self->state.N = evaluate(self->state);

                            ////////////////////////////
                            if(self->state.num.isprim == true)
                            {
                                self->state.setOfPrimFactors.insert(self->state.num.p);
                                //                cerr << " Neue Primzahl gefunden: The Primfactors are: ";
                                //                printSet( self->state.setOfPrimFactors);
                                cerr << "Found new Primfaktor: The new Primfaktor is: " << self->state.num.p << endl;
                            }

                                //wenn nicht dann insert p in Faktoren
                            else
                            {
                                self->state.setOfFactors.insert(self->state.num.p);
                            }
                            ...
                            for(it= self->state.setOfFactors.begin(); it!= self->state.setOfFactors.end(); ++it)
                            {
                                int512_t zwischenspeicher = *it;

                                // dividiere durch num.p bis != 0
                                while (zwischenspeicher % self->state.num.p == 0)
                                {
                                    zwischenspeicher/= self->state.num.p;
                                }
                                // insert das neue N
                            ...
                            self->state.setOfFactors.erase(self->state.num.N);
                            ...
                            {
                                if(self->state.done == false) {
                                    //  Stop timers
                                    self->state.done = true;
                                    double wallEndTime = get_wall_time();
                                    double cpuTime;
                                    double wallTime;
                                    wallTime = wallEndTime - self->state.wallStartTime;
                                    cpuTime = wallTime;

                                    //TODO: anzahlworker ersetzen
                                    cerr << "Number of Runs: " << self->state.num.cycles * NUM_WORKERS << endl;
                                    cerr << "Wall Time = " << wallTime << endl;
                                    cerr << "CPU Time  = " << cpuTime * NUM_WORKERS << endl;
